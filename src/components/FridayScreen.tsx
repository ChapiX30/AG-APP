...
import {
  ...
  Menu, Star, Eye, Settings, Zap, Target, Clock as ClockIcon, AlertCircle, Sun, Moon,
  Phone as PhoneIcon, Mail, Link as LinkIcon, DollarSign, Sigma
} from "lucide-react";
import clsx from "clsx";
import { useNavigation } from "../hooks/useNavigation";
import SidebarFriday from "./SidebarFriday";
import {
  collection, onSnapshot, doc, setDoc, updateDoc, getDoc, runTransaction
...
const BOARD_DOC_ID = "principal"; // <- cámbialo si quieres otro tablero
...
const COLUMN_TYPE_CATEGORIES = [
  {
    ...
    types: [
      ...
      { key: "number", icon: <Hash size={18} className="text-cyan-400" />, label: "Número", description: "Valores numéricos" },
...
function evalFormula(formula: string, row: Record<string, any>) {
  if (!formula) return "";
  // Reemplaza [campo] por su valor numérico y permite +-*/()., espacios
  let expr = formula.replace(/\[([^\]]+)\]/g, (_, key) => String(safeNumber(row[key.trim()])));
  // seguridad básica: solo números, operadores y paréntesis
...
function renderCell(type, value, row = {}, setFile = null, col = null) {
  ...
  if (type === "checkbox") {
    return (
      <div className="flex items-center">
        <input type="checkbox" checked={!!value} disabled className="w-4 h-4 accent-[#0073ea] rounded border-2 border-[#d0d4e4]" />
      </div>
    );
  }
  if (type === "progress") {
  ...
  if (type === "file") {
    return (
      <div className="inline-flex items-center gap-2 text-[#0073ea] hover:text-[#005bb5]">
        ...
        {setFile &&
          <input type="file" style={{ display: "none" }} id={`file-upload-${row.id}`} onChange={e => setFile && setFile(e, row)} />}
  ...
  if (type === "phone") {
    return value ? <a href={`tel:${value}`} className="text-[#0073ea] hover:underline">{value}</a> : <span className="text-[#676879] text-sm">-</span>;
  }
  if (type === "email") {
    return value ? <a href={`mailto:${value}`} className="text-[#0073ea] hover:underline">{value}</a> : <span className="text-[#676879] text-sm">-</span>;
  }
  if (type === "link") {
    if (!value) return <span className="text-[#676879] text-sm">-</span>;
  ...
  if (type === "time") {
    return value ? <span className="text-sm">{value}</span> : <span className="text-[#676879] text-sm">-</span>;
  }
  if (type === "currency") {
    return <span className="font-mono text-sm font-medium text-[#323338]">{formatCurrency(value, (col?.currency || "MXN"))}</span>;
  }
  if (type === "formula") {
  ...
  if (type === "tags") {
    ...
    return (
      <div className="flex flex-wrap gap-1">
        {tags.map((t, i) => (
          <span key={i} className="px-2 py-0.5 rounded-full text-xs bg-[#e6f0ff] text-[#0d47a1] border border-[#cfe0ff]">{t}</span>
        ))}
      </div>
    );
  }
...
function renderEditor(col, value, setValue, onSave) {
  ...
  switch (col.type) {
    case "status":
      return (
        <select {...commonProps} value={value ?? ""} onChange={e => setValue(e.target.value)}>
          {STATUS_OPTIONS.map(opt => <option key={opt.value} value={opt.value}>{opt.value}</option>)}
        </select>
      );
    case "priority":
      return (
    ...
    case "checkbox":
      return (
        <input type="checkbox" checked={!!value} onChange={e => setValue(e.target.checked)} onBlur={onSave} className="w-5 h-5 accent-blue-500 rounded" autoFocus />
      );
    case "date":
    ...
    case "formula":
      // Solo lectura en la tabla; si quieres editar fórmula, hazlo desde menú de columna
...
export default function FridayScreen() {
  ...
  // Estado de datos del tablero (Firestore)
  ...
  // Carga/sync de usuarios (responsables)
  useEffect(() => {
  ...
  // Firestore: sync en tiempo real del tablero
  useEffect(() => {
  ...
  useEffect(() => {
    const t = setTimeout(async () => {
      ...
      try {
        ...
        // si falla (p.ej. doc no existe), intenta setDoc
  ...
  const onDragEnd = (result) => {
    ...
    if (type === "row") {
      const fromIdx = +source.droppableId.replace("group-", "");
      const toIdx = +destination.droppableId.replace("group-", "");
      const sourceGroup = groups[fromIdx];
      const destGroup = groups[toIdx];

      const sourceRows = Array.from(sourceGroup.rows);
      const [moved] = sourceRows.splice(source.index, 1);

      if (fromIdx === toIdx) {
        // mismo grupo, reordenar
        sourceRows.splice(destination.index, 0, moved);
        setGroups(gs => {
          const ngs = [...gs];
          ngs[fromIdx] = { ...ngs[fromIdx], rows: sourceRows };
          return ngs;
        });
      } else {
        // entre grupos
        const destRows = Array.from(destGroup.rows);
        destRows.splice(destination.index, 0, moved);
        setGroups(gs => {
          const ngs = [...gs];
          ngs[fromIdx] = { ...ngs[fromIdx], rows: sourceRows };
          ngs[toIdx] = { ...ngs[toIdx], rows: destRows };
          return ngs;
        });
      }
      setSaveTick(t => t + 1);
                ...
                <Draggable draggableId={group.id} index={gidx} key={group.id}>
                  {(prov, snap) => (
                    <div
                      ref={prov.innerRef}
                      {...prov.draggableProps}
                      className={clsx("group-container transition-transform", snap.isDragging && "rotate-[0.5deg]")}
                      style={{ transform: snap.isDragging ? "scale(1.01)" : undefined }}
                    >
                      {/* Header grupo */}
                      <div className={clsx(
                      ...
                      {!group.collapsed && (
                        <Droppable droppableId={"group-" + gidx} type="row">
                          {(provRow, snapRow) => (
                            <div
                              ref={provRow.innerRef}
                              {...provRow.droppableProps}
                              className={clsx("bg-white border border-[#e6e9ef] rounded-b-lg overflow-hidden shadow-sm",
                                snapRow.isDraggingOver && "ring-2 ring-[#0073ea]/30")}
                            >
                                ...
                                <table className="w-full min-w-[1200px]">
                                  <thead>
                                    ...
                                    <Droppable droppableId="columns-droppable" direction="horizontal" type="column">
                                      {(providedDroppable, snapCols) => (
                                        <tr
                                          ref={providedDroppable.innerRef}
                                          {...providedDroppable.droppableProps}
                                          className={clsx("bg-[#f8f9fd] border-b border-[#e6e9ef]",
                                            snapCols.isDraggingOver && "outline outline-2 outline-[#0073ea]/40")}
                                        >
                                    ...
                                    {group.rows.filter(filterRow).map((row, ridx) => (
                                      <Draggable draggableId={row.id} index={ridx} key={row.id}>
                                        {(provR, snapR) => (
                                          ...
                                          >
                                            <td className={clsx("px-4 py-3", "sticky left-0 bg-white z-10 border-r border-[#e6e9ef]")}>
                                              <input
                                                type="checkbox"
        ...
        {selectedRows.length > 0 && (
          <div className="fixed bottom-8 left-1/2 -translate-x-1/2 bg-white border border-[#e6e9ef] rounded-xl shadow-lg px-6 py-4 flex items-center gap-6 z-50 animate-slideUp">
            ...
            <div className="flex items-center gap-4">
              <button
                className="flex flex-col items-center gap-1 text-[#676879] hover:text-[#0073ea] transition-all"
                title="Duplicar"
                onClick={() => {
                  setGroups(gs => gs.map((g, gidx) => ({
                    ...g,
  ...
  /* ========== Kanban & Calendario (placeholder visual) ========== */
    ...
    return (
      <div className="py-8 flex justify-center">
        <div className="rounded-2xl border border-slate-700 bg-slate-900 shadow-2xl p-10 text-center max-w-xl w-full">
          ...
          <p className="text-slate-400 mb-2">Lista para integrar <code>react-big-calendar</code> o similar.</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <GlassModal open={massEditModal} onClose={() => setMassEditModal(false)} title="Edición masiva">
        <div className="space-y-4">
          ...
          <button className="mt-3 w-full py-2 rounded bg-blue-600 text-white font-bold" onClick={handleMassEdit}>
            Aplicar cambios
          </button>
        </div>
      </GlassModal>
    ...
    return (
      <GlassModal open={showFilterModal} onClose={() => setShowFilterModal(false)} title="Filtrar tablero">
        <div className="space-y-4">
          ...
          <button className="mt-3 w-full py-2 rounded bg-blue-600 text-white font-bold" onClick={() => setShowFilterModal(false)}>
            Aplicar
          </button>
        </div>
      </GlassModal>
