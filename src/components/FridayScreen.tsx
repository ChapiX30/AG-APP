...
// --- GENERADOR DE COLOR AUTOMÁTICO (PASTELES) ---
...
const BoardRow = React.memo(({ row, columns, color, isSelected, onToggleSelect, onUpdateRow, metrologos, clientes, onDragStart, onDrop, onDragEnd, userRole, onOpenComments, index, groupId }: any) => {
    ...
    return (
        <div 
            id={`row-${row.docId}`} 
            className="flex border-b border-[#d0d4e4] group transition-colors h-[40px] hover:bg-gray-50/80" 
            draggable="true" 
            onDragStart={(e) => onDragStart(e, { type: 'row', index, id: row.docId, groupId })}
            onDragOver={(e) => e.preventDefault()} 
            onDragEnd={onDragEnd} 
            onDrop={(e) => { e.stopPropagation(); onDrop(e, { type: 'row', index, id: row.docId, groupId }); }} 
        ...
        >
            ...
            <div className="w-[40px] flex-shrink-0 border-r border-[#d0d4e4] sticky left-1.5 z-20 flex items-center justify-center" style={{ backgroundColor: rowBackgroundColor }}>
                 <div className="w-full h-full flex items-center justify-center relative group/control">
                    <button onClick={() => onOpenComments(row)} className={clsx("p-1 rounded hover:bg-black/10 text-gray-400 hover:text-blue-600 transition-colors", isSelected ? "hidden" : "block")}><MessageSquare size={14} /></button>
...
const ColumnOptions = ({ colKey, onClose, onSort, onHide, onRename, onPermissions, currentLabel }: any) => {
...
const FridayScreen: React.FC = () => {
    ...
    const handleUpdateRow = useCallback(async (rowId: string, key: string, value: any) => {
        ...
        try { await updateDoc(doc(db, "hojasDeTrabajo", rowId), { [key]: value, lastUpdated: new Date().toISOString() }); } catch (error) { console.error("Error updating row:", error); }
    }, []);
                    ...
                    <div className="inline-block min-w-full pb-32">
                        ...
                        <div className="flex border-b border-[#d0d4e4] sticky top-0 z-30 bg-white shadow-sm h-[36px]">
                            ...
                            {columns.filter(c => !c.hidden).map((col, index) => {
                                ...
                                >
                                    <span className="truncate flex items-center gap-1 flex-1 justify-center">{col.label} {isLocked && <Lock className="w-2.5 h-2.5 text-gray-300" />}</span>
                                    ...
                                    {activeColumnMenu === col.key && (
                                        <ColumnOptions colKey={col.key} currentLabel={col.label} onClose={() => setActiveColumnMenu(null)} onSort={handleSort} onHide={handleHide} onRename={handleRename} onPermissions={(k:string) => handleOpenPermissions(window.event, k)}/>
                                    )}
                                </div>
                            )})}
