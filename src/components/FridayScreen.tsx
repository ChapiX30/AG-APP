...
const CommentsPanel = ({ row, onClose, canPost }: { row: WorksheetData; onClose: () => void; canPost: boolean }) => {
    ...
    return (
        <div className="fixed inset-y-0 right-0 w-[min(20rem,100vw)] bg-white shadow-2xl z-[120] flex flex-col border-l border-gray-200 animate-in slide-in-from-right duration-300">
...
const ToastContainer = ({ toasts, removeToast }: { toasts: any[], removeToast: (id: string) => void }) => {
    return (
        <div className="fixed bottom-6 right-6 z-[120] flex flex-col gap-2 pointer-events-none">
            {toasts.map(toast => (
                <div key={toast.id} className="pointer-events-auto bg-gray-900/90 backdrop-blur text-white px-4 py-3 rounded-xl shadow-2xl flex items-start gap-3 animate-in slide-in-from-right fade-in duration-300 w-[min(20rem,calc(100vw-2rem))] max-w-sm border border-white/10">
...
const AGBotWidget = ({ thoughts }: { thoughts: AGBotThought[] }) => {
    ...
    return (
        <div className="relative">
            ...
            {open && (
                <div className="absolute right-0 top-12 w-[min(20rem,calc(100vw-1.5rem))] bg-white rounded-2xl shadow-2xl border border-purple-100 z-[80] overflow-hidden animate-in fade-in slide-in-from-top-4">
                        ...
                        <div className="flex border-b border-[#e6e9ef] sticky top-0 z-30 bg-[#f5f6f8] h-[40px]">
                            ...
                            {visibleColumns.map((col, index) => {
                                ...
                                <div key={col.key} draggable={canEditBoard && !col.sticky && !isResizing} onDragStart={(e) => onDragStart(e, { type: 'column', index })} onDragOver={(e) => e.preventDefault()} onDragEnd={onDragEnd} onDrop={(e) => onDrop(e, { type: 'column', index })} style={style} className={clsx("px-3 text-[12px] font-semibold text-[#8B8D8C] flex items-center justify-center border-r border-[#e6e9ef] hover:bg-[#eceef3] select-none bg-[#f5f6f8] group hover:text-[#323338] transition-colors relative", col.sticky ? "shadow-[1px_0_3px_rgba(0,0,0,0.04)]" : "cursor-pointer")}>
                                    ...
                                    <button onClick={(e) => { e.stopPropagation(); setActiveColumnMenu(activeColumnMenu === col.key ? null : col.key); }} className="p-1.5 rounded hover:bg-gray-200 opacity-100 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity absolute right-0.5 min-h-8 min-w-8 flex items-center justify-center"><MoreHorizontal className="w-3.5 h-3.5 text-gray-500" /></button>
