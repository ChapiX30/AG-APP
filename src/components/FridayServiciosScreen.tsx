...
import { autoStartServiciosIfDue, formatHoraFin, hasCelesticaAsignacionPendienteForUser, isCelesticaAsignacionPendiente, isUserInPersonas } from '../utils/servicioAutomation';
      ...
      <div onClick={onClick} className={`group bg-white rounded-xl border border-gray-200 p-3 shadow-sm hover:border-[#2464A3]/50 hover:shadow-lg transition-all cursor-pointer flex flex-col sm:flex-row gap-3 items-center relative ${cardTone}`}>
        ...
        <div className="flex-1 min-w-0 w-full">
          ...
          {showCelesticaCues && cueUserId && (
            <CelesticaSitioCardCues uid={cueUserId} enabled compact onOpen={onOpenCelesticaCue} />
          )}
        </div>
...
const ServiceDetailModal = ({ isOpen, onClose, service, usuarios, onEdit, onDelete, onViewFile, canEdit, currentUser, currentUserIds, onOpenCelesticaCue }: any) => {
    ...
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center sm:p-4 bg-gray-900/60 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="bg-white w-full max-w-2xl sm:rounded-2xl rounded-t-2xl shadow-2xl flex flex-col max-h-[95vh] overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-6 bg-white custom-scrollbar">
          {isCelesticaAsignacionPendiente(service) && isUserInPersonas(service.personas, currentUserIds) && currentUser?.id && (
            <CelesticaSitioDetailCues uid={currentUser.id} enabled onOpen={onOpenCelesticaCue} />
          )}
...
const FridayServiciosScreen: React.FC = () => {
  ...
  const currentUserIds = useMemo(() => {
    ...
    if (currentUserData?.id) ids.add(currentUserData.id);
    return [...ids];
  }, [authUser?.uid, currentUserData?.id]);
  const hasCelesticaPendiente = useMemo(
    () => hasCelesticaAsignacionPendienteForUser(servicios, currentUserIds),
    [servicios, currentUserIds]
  );
  const openCelesticaCue = useCallback((screen: string) => {
    navigateTo(screen);
  }, [navigateTo]);
    ...
    <div className="flex h-full min-h-0 flex-1 flex-col bg-[#eef2f7] font-sans text-slate-900 overflow-hidden">
      <main className="flex-1 flex flex-col h-full min-w-0 relative">
        ...
        <header className="bg-white border-b border-slate-200 z-10 sticky top-0 shadow-sm">
          ...
          <CelesticaSitioCueBar
            uid={currentUserId}
            enabled={hasCelesticaPendiente}
            onOpen={openCelesticaCue}
          />
        </header>
