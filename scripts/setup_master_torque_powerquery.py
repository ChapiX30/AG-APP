#!/usr/bin/env python3
"""
Configura Formato Torque.xlsm para sincronizarse automáticamente al abrir.

Power Query actualiza:
- obtenerDatosExcel: hojas de trabajo AGPT + datos completos del cliente
- BD_Clientes: catálogo de clientes
- BD_Patrones: catálogo de patrones

No modifica cálculos, lecturas ni incertidumbres.
Requisito: Excel instalado y Formato Torque.xlsm cerrado.
"""
...
MASTER = Path(r"C:\Users\AG\Desktop\FORMATOS AG\Formato Torque.xlsm")
API_BASE = (
...
M_HISTORIAL = f'''let
    ...
    AnioActual = Text.End(Text.From(Date.Year(DateTime.LocalNow())), 2),
    HistorialAGPT = Table.SelectRows(
        HistorialBase,
        each (Text.StartsWith(Text.Upper(Text.From([certificado])), "AGPT-")
              or Text.StartsWith(Text.Upper(Text.From([certificado])), "AGPTT-"))
            and Text.EndsWith(Text.From([certificado]), "-" & AnioActual)
    ),
    ...
    HistorialClave = Table.AddColumn(
        HistorialAGPT, "_clienteKey", each NormalizarNombre([cliente]), type text
    ),
...
M_PATRONES = f'''let
    ...
    SoloTorque = Table.SelectRows(
        ...
        in
            Text.Contains(d, "TORSIONAL") or Text.Contains(d, "TORQ") or Text.Contains(d, "HIOS")
            or List.Contains({{"AG-051", "AG-025", "AG-014", "AG-021", "AG-052", "AG-013", "AG-037"}}, id)
    ),
...
def configure_calculation(excel, wb) -> None:
    ...
    try:
        ...
        pass
    try:
        # ForceFullCalculation=True hace lentísimo cada recálculo y el abrir.
...
def main() -> int:
    ...
    try:
        ...
        query_defs = (
            ("AG_API_Historial_Torque", M_HISTORIAL, HIST_SHEET, "AG_Historial"),
            ("AG_API_Clientes_Torque", M_CLIENTES, CLIENTES_SHEET, "AG_Clientes"),
            ("AG_API_Patrones_Torque", M_PATRONES, PATRONES_SHEET, "AG_Patrones"),
        )
