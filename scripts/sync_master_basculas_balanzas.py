# -*- coding: utf-8 -*-
"""
Sincroniza Formato Básculas y Balanzas.xlsm con la app (prefijo AGM).
...
Uso:
  python scripts/sync_master_basculas_balanzas.py
  python scripts/sync_master_basculas_balanzas.py --archivo "C:\\...\\Formato Básculas y Balanzas.xlsm"
"""
...
DEFAULT_MASTER = Path(r"C:\Users\AG\Desktop\FORMATOS AG\Formato Básculas y Balanzas.xlsm")
...
def fetch_json(url: str) -> dict:
    req = urllib.request.Request(url, headers={"User-Agent": "AG-MasaSync/1.0"})
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Sync master Básculas/Balanzas")
    parser.add_argument("--archivo", type=Path, default=DEFAULT_MASTER)
    parser.add_argument("--prefijo", default="AGM")
    ...
    patrones = [p for p in (data.get("patrones") or []) if es_patron_masa(p)]
    historial, n_join = enrich_historial(historial, clientes)
    print(f"  historial {len(historial)}  clientes {len(clientes)}  patrones masa {len(patrones)}")
    ...
    try:
        ...
        print("Uso: en CALCULOS pon D4=AGM  E4=número  F4=año (ej. AGM | 36 | 26)")
        return 0
