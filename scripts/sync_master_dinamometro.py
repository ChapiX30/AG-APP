# -*- coding: utf-8 -*-
"""
Sincroniza Formato Dinamometro Unificado.xlsm con la app (prefijo AGF/AGFT).
...
Uso:
  python scripts/sync_master_dinamometro.py
  python scripts/sync_master_dinamometro.py --archivo "C:\\...\\Formato Dinamometro Unificado.xlsm"
"""
...
DEFAULT_MASTER = Path(r"C:\Users\AG\Desktop\FORMATOS AG\Formato Dinamometro Unificado.xlsm")
...
FUERZA_IDS = {"AG-017", "AG-018", "AG-019", "AG-020", "AG-022", "AG-023", "AG-040"}


def fetch_json(url: str) -> dict:
    req = urllib.request.Request(url, headers={"User-Agent": "AG-FuerzaSync/1.0"})
...
def es_patron_fuerza(p: dict) -> bool:
    no = str(p.get("noControl") or "").strip().upper()
    if no in FUERZA_IDS:
        return True
    desc = str(p.get("descripcion") or "").upper()
    return any(k in desc for k in ("MASA", "PESA", "CELDA", "CARGA", "FUERZA", "DINAM"))


def es_cert_fuerza(cert: str) -> bool:
    c = str(cert or "").strip().upper()
    return c.startswith("AGF-") or c.startswith("AGFT-")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Sync master Dinamómetro / Fuerza")
    parser.add_argument("--archivo", type=Path, default=DEFAULT_MASTER)
    parser.add_argument("--prefijo", default="AGF")
    ...
    # Preferir filtro local AGF/AGFT; si la API acepta prefijo, también lo mandamos
    historial_all = data.get("historial") or []
    historial = [r for r in historial_all if es_cert_fuerza(r.get("certificado"))]
    if not historial and args.prefijo:
        sep = "&" if "?" in url else "?"
        url_pref = f"{url}{sep}prefijo={args.prefijo}"
        print(f"Sin AGF en respuesta general; reintentando\n  {url_pref}")
        data2 = fetch_json(url_pref)
        historial = data2.get("historial") or historial_all
        if data2.get("clientes"):
            data["clientes"] = data2["clientes"]
        if data2.get("patrones"):
            data["patrones"] = data2["patrones"]

    clientes = data.get("clientes") or []
    patrones = [p for p in (data.get("patrones") or []) if es_patron_fuerza(p)]
    historial, n_join = enrich_historial(historial, clientes)
    print(f"  historial fuerza {len(historial)}  clientes {len(clientes)}  patrones {len(patrones)}")
    ...
    try:
        ...
        print("Uso: en CALCULOS pon D4=AGF  E4=número  F4=año (ej. AGF | 52 | 26)")
        return 0
