# -*- coding: utf-8 -*-
"""Angle meter: historial libre (todos los prefijos) + AutoSync sin filtro."""
...
def main() -> int:
    ...
    hist_rows = []
    for h in hist_raw:
        cert = str(h.get("certificado") or "").strip()
        if not cert or cert in ("-",):
            continue
    ...
    print("Filas historial:", len(hist_rows))
    ...
    try:
        ...
        setup.set_module(wb.VBProject, "AG_AutoSync", setup.build_autosync_vba())
        print("AG_AutoSync actualizado (sin filtro)")
        ...
        d4, e4, f4 = calc.Range("D4").Value, calc.Range("E4").Value, calc.Range("F4").Value
        print("Cert actual:", d4, e4, f4)
